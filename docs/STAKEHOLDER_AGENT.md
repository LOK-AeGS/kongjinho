...
python -m scripts.run_stakeholder --ask-key --max-queries 8
```
...
python -m scripts.run_stakeholder --as-of 2026-09-21 --allowed-domain github.com --allowed-domain arxiv.org
```
...
```bash
python -m scripts.run_stakeholder --offline-fixture tests/agents/stakeholder/fixtures/web_replay.json --as-of 2026-09-21
python -m unittest tests.agents.stakeholder.test_stakeholder -v
```
...
기존 graph/build.py는 이전 구조용이며 새 그래프가 구현된 것으로 간주하면 안 됩니다.
